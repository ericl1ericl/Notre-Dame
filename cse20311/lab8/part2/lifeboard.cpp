...
Lifeboard::Lifeboard() // creates board and border
{ 
   for (int row = 0; row < 42; row++) { // 42 rows so that two can be border
...
void Lifeboard::display() // displays everything, including border
{
...
int Lifeboard::remC(int x, int y) // replaces cell with blank space
{
   ...
   if ((x < 41) && (x >= 1) && (y < 41) && (y >= 1)) { // checks if in boundary
      board[y][x] = ' '; // replaces cell with blank space
   }
   else {
...
int Lifeboard::addC(int x, int y) // replaces cell with live space
{
   x++; // changes x so valid user inputs of 0-39 affect positions 1-40 on board
   y++; // changes y so valid user inputs of 0-39 affect positions 1-40 on board
   if ((x < 41) && (x >= 1) && (y < 41) && (y >= 1)) { // checks if in boundary
...
int Lifeboard::isState(int x, int y) // returns if (x,y) should be dead or alive
{
